			...
			<div><!-- 스크롤 처리되는 div -->
				...
				<div class="content_in">
					...
					<form name="noticeForm" id="noticeForm" method="post">
						<input type="hidden" name="pageNum"> <input type="hidden" name="bbsCate" value="1">
						<!--2015.08.06 이미지를 텍스트로변경-->
						<h2>A/S 문의</h2>
						<a class="radius_btn right_btn mb15_w40" href="write" >
							<span>글 쓰기</span>
						</a>
						...
						<table class="tb0_style">
							...
							<tbody>
								<c:forEach var="boardas" items='${list}'>	<%-- 이 표현식은 null값이 들어가도 예외가 발생되지 않는다. --%> 
									<tr>
										<td>${boardas.bano}</td>
										<td><a href="info?bano=${boardas.bano}">${boardas.batitle}</a></td>
										<td>${boardas.bawriter}</td>
										<td>${boardas.badate}</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
