	...
	<script type="text/javascript">
		...
		$(function(){
			$("#btn_confirm").on("click", function(){
				if($('#fbcategory').val().replace(/ /g, '') == ""){
					alert("카테고리를 작성해 주세요.");
				}else if($('#fbtitle').val().replace(/ /g, '') == ""){
					alert("제목을 작성해 주세요.");
				}else if($('#fbcontent').val().replace(/ /g, '') == ""){
					alert("내용을 작성해 주세요.");
				}else{
					$("#write").submit();
				}
				...
				<div class="content_in">
					...
					<form id="write" method="post">
						<table class="tb2_style">
							...
							<thead>
								<tr>
									<th class="t_style">카테고리</th>
									<th>
										<input type="text" id="fbcategory" name="fbcategory" class="form-control" style="height: 28px"
										 placeholder="회원정보=member , 제품문의=product, 사이트이용=sitr, 기타=etc">
									</th>
								</tr>
							</thead>
							...
							<tbody>
								<tr>
									<td class="t_style">제목</td>
									<td style="padding: 10px 0;">
									<input type="text" id="fbtitle" name="fbtitle" class="form-control" style="height: 28px">
								</tr>
								<tr style="height: 50vh">
									<td class="t_style">내용</td>
									<td>
										<textarea name="fbcontent" id="fbcontent" class="form-control" style="height: 90%"></textarea>
