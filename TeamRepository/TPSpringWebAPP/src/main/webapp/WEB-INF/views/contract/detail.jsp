...
/* 			margin: auto; */
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			<%-- 날짜 선택 시 페이지 전환 --%>	// form 으로 날짜, 룸id를 넘겨 페이지 전환 하려고 했으나 실패.....
			$("#cday").on("change", function(){
				var crid = $("#selectbox").val();
				var cday = $("#cday").val();
				
				
				$("#detailIframe", parent.document)[0].src = "<%=pageContext.getServletContext().getContextPath()%>/contract/detail?crid=" + crid + "&cday=" + cday;
				console.log($("#detailIframe", parent.document)[0].src);
				//$("#detailIframe", parent.document)[0].contentWindow.location.reload(true);
			})
		...
		function clickOption() {
			...
			$("#detailIframe", parent.document)[0].src = "<%=pageContext.getServletContext().getContextPath()%>/contract/detail?crid=" + value + "&cday=" + cday;
									...
									<td>
										...
										<div style="width:180px;">
										    <select name="selectbox" size="7"  id="selectbox" style="width:100px; padding-left:5px;">
										    	<c:forEach var="roomList" items="${list}">
										    		<option value="${roomList.rid}" onclick="clickOption()" <c:if test="${roomList.rid == rid}">selected</c:if>>${roomList.rname}</option>
										        </c:forEach>
										    </select>
										</div>
									</td>
...
<!-- 										<h3>룸<br /> 계약 정보 </h3> -->
										<div class="alert alert-info">${roomName}</div>
										<ul id="contract">
							...
							<tfoot>
								<tr>
									<td colspan="3">
										<%-- 날짜 선택 버튼 --%>
										<input id="cday" type="date" class="form-control" style="width: 170px;" value="${cday}">
										<input type="hidden" id = "crid" name = "crid" value="${rid}">					
										<%-- 라인 차트 --%>
