...
public class Ex04 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			OutputStream out = new FileOutputStream("C:\\test\\goods_out.csv");
...
//							goods_count[selectNum] = goods_count[selectNum] - buyProductCount[buyCount];
							// 상품별 수량 누적값
							totalCount[selectNum] += buyProductCount[buyCount];
							// 상품별 가격 누적값
							productResult[selectNum] += result[buyCount];
							// 전체 총계 구하기
...
//						}
				...
				System.out.println();
				System.out.println("총계 : " + loopResult);
				System.out.println();
			...
			System.out.println();
			System.out.println("==== [마지막 총 집계] ====");
			System.out.println("상품명\t가격\t수량\t소계");
			for(int i=0; i<count; i++){
				// 상품별 누적값이 0이 아닐 경우에만 출력
				if(productResult[i] != 0){
					String dataStr = null;
					
					System.out.print(goods_name[i] + "\t");
					System.out.print(goods_price[i] + "\t");
					System.out.print(totalCount[i] + "\t");
					System.out.println(productResult[i]);
					
					if(i == 0)
						 dataStr = "";
					dataStr = goods_name[i] + ",";
					dataStr += goods_price[i] + ",";
					dataStr += totalCount[i] + ",";
					dataStr += productResult[i] + "\n";
