...
public class Lec03 {
	public static void main(String[] args) throws IOException {
		...
		while((readtxt=br.readLine())!=null){
			...
			if(field[5].trim().equals("SKT") || field[5].trim().toLowerCase().equals("sk텔레콤") || field[5].trim().equals("SK")){
				if(field[5].trim() != "SKT")
					field[5] = "SKT";
				bw1.write(readtxt);bw1.newLine();
			}else if(field[5].trim().equals("KT")){
				bw2.write(readtxt);bw2.newLine();
			}else if(field[5].trim().equals("LGU+") || field[5].trim().equals("LGT") || field[5].trim().equals("LU G+")){
				if(field[5].trim() != "LGU+"){
					field[5] = "LGU+";
				}
				bw3.write(readtxt);bw3.newLine();
