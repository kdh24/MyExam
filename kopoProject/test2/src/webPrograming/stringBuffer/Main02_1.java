...
//		}
		...
		for(int i = 0; i<maxLoop; i++){
			...
			if (i == maxLoop - 1) {
				if(Main02_1.iPerson % pageNum != 0){
					pageNum= Main02_1.iPerson % pageNum;
				}
			}
			System.out.println(dataStart);
//			dataSet(dataStart, dataStart+pageNum); 		// 데이터 셋팅
...
//				dataCouEut.printf(" %d\n", maxLoop);
	...
	//데이터 만들기
	public static void dataSet(int dataStart, int dataEnd){
		String name = null;
		int kor = 0;
		int eng = 0;
		int mat = 0;
		...
		if (dataStart <= iPerson) {
			for (int i = dataStart; i < dataEnd; i++) {
				...
				ArrayOneRec.add(new OneRec(i + 1, name, kor, eng, mat)); 
